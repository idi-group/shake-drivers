#undef _DEBUG /* Link with python2x.lib and not python2x_d.lib */
...
#include <Python.h>
#endif
#include "pyshakecpp.h"

static PyObject* pyshakecpp_ex;
...
static void SHAKE_CALLBACK main_audio_callback(shake_device* dev, short* mic_samples, int mic_len, short* playback_samples, int playback_len) {
	...
	//printf("pyshakecpp_audio_callback\n");
	...
	for(i=0;i<devicelist_count;i++) {
		if(devicelist[i] == dev && audio_callbacks[i][1] != NULL) {
			...
			//printf("pyshakecpp_audio_callback for device %d, %d samples for mic (%d), %d samples for playback (%d)\n", i, mic_len, mic_samples == NULL, playback_len, playback_samples == NULL);
...
PYSHAKECPP_EXPORT void initpyshakecpp(void) {
	PyObject* mod = Py_InitModule("pyshakecpp", pyshakecpp_methods);
	PyEval_InitThreads();
	char newExceptionName[] = "pyshakecpp.error";
	pyshakecpp_ex = PyErr_NewException(&newExceptionName[0], NULL, NULL);
	Py_INCREF(pyshakecpp_ex);
	PyModule_AddObject(mod, "PyshakeError", pyshakecpp_ex);
}
...
static PyObject* pyshakecpp_info_retrieve(PyObject* self, PyObject* args) {
	...
	printf("pyshakecpp_info_retrieve...\n");
